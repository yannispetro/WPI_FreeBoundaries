...
%     
    ...
    PDF_Yt = interp1([0.99*YYYmc(1),Y_orig,1.01*YYYmc(end)],[0,PDF_Yt_orig,0],YYYmc);
...
ylabel('$$x_{97}$$','Interpreter','latex','FontSize',40)
zlabel('$\dot{x}_{97}$','Interpreter','latex','FontSize',40)
...
%     
    ...
    PDF_Yt = interp1([0.99*YYY(1),Y_orig,1.01*YYY(end)],[0,PDF_Yt_orig,0],YYY);
    for i = 1:N
        V_Xt(:,k,i) = PDF_Xt;
        V_Yt(i,k,:) = PDF_Yt;
    end
...
% print(figMC,'nanoJointMCS_time','-depsc','-r1000')
% print(figWPI,'nanoJointWPI_time','-depsc','-r1000')
